...
# -------------------
enum Type {NOT_ASSIGNED, EASY, MEDIUM, HARD, BOSS}
...
func update_room_connections():
	...
	for room in rooms:
		...
		var north_room = occupied.get(pos + Vector2i.UP)
		if north_room:
			north_open = room.has_north_door and north_room.has_south_door

		var east_room = occupied.get(pos + Vector2i.RIGHT)
		if east_room:
			east_open = room.has_east_door and east_room.has_west_door

		var south_room = occupied.get(pos + Vector2i.DOWN)
		if south_room:
			south_open = room.has_south_door and south_room.has_north_door

		var west_room = occupied.get(pos + Vector2i.LEFT)
		if west_room:
			west_open = room.has_west_door and west_room.has_east_door
